...
for i in range(n):
    drift = min(
        i * 0.002, 5
    )  # Drift con saturación (La máquina se degrada, pero no indefinidamente)
    ...
    if not anomalia_activa and np.random.rand() < 0.03:
        ...
        for _j in range(duracion_restante):
            tipo = np.random.choice(
                ["temp_alta", "presion_baja", "ciclo_largo", "multiple"],
                p=[0.4, 0.3, 0.2, 0.1],
            )
            ...
            if anomalia_activa:
                if tipo == "temp_alta":
                    ...
                    presion_iny = presion_iny + offset_presion
                    presion_iny = (
                        np.clip(presion_iny, presion_min, presion_max) + offset_presion
                    )
                    presion_iny = max(presion_iny, presion_min + np.random.normal(0, 5))

                    tiempo_ciclo = tiempo_ciclo + offset_ciclo
                    tiempo_ciclo = (
                        np.clip(tiempo_ciclo, ciclo_min, ciclo_max) + offset_ciclo
                    )
