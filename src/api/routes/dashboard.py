...
lifespan). ``/overview``, ``/timeline``, ``/sensor-health`` and ``/incidents``
are ``{meta, data}`` view endpoints that fail closed on invalid lineage. No
endpoint reads artifacts per request: ``/meta`` and ``/overview`` serve static
pins shared via ``src.api.services.view_meta``, while ``/timeline``,
``/sensor-health`` and ``/incidents`` read the pinned run's artifacts once at
