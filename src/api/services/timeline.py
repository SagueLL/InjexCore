...
# artifacts; the p95 warning threshold is the §8 calibration open item —
...
def _daily_p95(scores: pd.DataFrame) -> pd.Series:
...
def _overlaps(days: pd.DatetimeIndex, frame: pd.DataFrame) -> np.ndarray:
...
def _day_statuses(
    ...
    warning = _overlaps(days, incidents[incidents["severity"] == "warning"]).any(
        axis=1
    ) | (p95.to_numpy() >= _WARNING_P95_THRESHOLD)
    statuses = np.select(
...
def _build_timeline(
    ...
    p95 = _daily_p95(scores)
    days = pd.DatetimeIndex(p95.index)
    incident_counts = _overlaps(days, incidents).sum(axis=1).tolist()
    statuses = _day_statuses(days, p95, incidents, drift_events)
