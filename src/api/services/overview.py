...
Assembles the dashboard-ready ``{meta, data}`` overview response. Serves
stable B1-verified canonical-run values — properties of the immutable pinned
run — and reads no artifacts per request (API contract §8 risk 1).
"""
...
# sensor-health summary, scenario_anomaly_rate_comparison) in a later slice.
...
_PROBLEMATIC_SENSOR_COUNT = 17  # not the demo's 23 (contract §8 recopy)
...
def _build_kpis() -> list[DashboardKpi]:
    return [
        ...
        DashboardKpi(
            ...
            value=_PROBLEMATIC_SENSOR_COUNT,
            description=(
...
def _build_insights() -> list[ExecutiveInsight]:
    return [
        ExecutiveInsight(
        ...
        ExecutiveInsight(
            ...
            body=(
                f"{_PROBLEMATIC_SENSOR_COUNT} sensors show warning, faulty or "
...
def build_overview_summary() -> DashboardSummary:
    """Build the overview ``data`` payload from the pinned canonical values."""
    return DashboardSummary(
        ...
        kpis=_build_kpis(),
        executive_insights=_build_insights(),
