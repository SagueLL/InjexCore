...
that return the column sets each data-layer module needs (Process,
...
DEFAULT_CLASSIFICATION = FEATURES_DIR / "variable_classification.csv"


@dataclass(frozen=True)
class ColumnGroups:
    """Materialised column sets used across data-layer modules."""
    ...
    def signal_columns(self) -> list[str]:
        ...
        return [
            c for c in self.all_columns if c not in self.metadata and c != "timestamp"
        ]
...
def load_groups(
    ...
    running_flags = [
        v for v in state if v.endswith("_running") or v.endswith("_direct")
    ]
