...
# Intelligence Layer — Iteration C close-out: Human Decision Overlay (encodes the
# reviewer's scoped inlet_hopper_points quarantine approval + Reference-v2 deferral
# as a read-only, run-versioned decision-support artifact; mutates/approves/refits/
# rescores NOTHING; manifest LAST). Run id: human-decision-v1-<UTC>.
#   → data/intelligence/forensics/human_decisions/<run_id>/ (flat forensics layout)
python -m src.intelligence --component human-decision           # via dispatcher
python -m src.intelligence --component human-decision --no-write # diagnostic only
python -m src.intelligence.human_decisions.run_human_decision_overlay  # direct
...
```
data_generation/ → preprocessing/{cleaning → time_series → feature_engineering → datasets} → intelligence/{behaviour → correlation·pca → anomaly → drift → incidents → reference → scoring-experiment → human-decision} → [src/models/] → apps/dashboard (Dashboard v0 UI)
...
| `src/intelligence/human_decisions/` | Done (v1) | Iteration C close-out — Human Decision Overlay. Encodes the reviewer's scoped `inlet_hopper_points` quarantine approval + Reference-v2 deferral as a run-versioned, read-only decision-support artifact under the flat `data/intelligence/forensics/human_decisions/<run_id>/` layout (run id `human-decision-v1-<UTC>`; manifest LAST). Keeps `artifact_state` / `human_review_state` / `applied_pipeline_state` / `dashboard_overlay_state` distinct; mutates nothing, approves no quarantine, refits/rescores nothing (forbidden dashboard actions: approve/apply/refit/create-v2/mutate-scores/alert). Authoritative `scope_start_row=137235` (`2024-09-17 16:23:22`), `human_reported_row=137237` preserved. `--component human-decision`. |
...
| `src/intelligence/__main__.py` | Done | Component dispatcher (`python -m src.intelligence --component behaviour\|correlation\|pca\|anomaly\|sensor-health\|drift\|incidents\|reference\|scoring-experiment\|human-decision`); registry maps each of the 10 component names to its `main`. Defaults to `behaviour`; forwards remaining flags to the component CLI. |
...
| `src/dashboard/` | Done (contract) | Read-only dashboard consumption contract + lineage validator. `contract.py` pins the canonical chain (`CANONICAL_RUN_ID`/`CANONICAL_BOM_RUN_ID`) and the component registry; `lineage.py`'s `validate_dashboard_chain(run_id, bom_run_id)` resolves every component's pinned run read-only (reusing `runs.is_completed_run`), fails closed on missing/wrong-component/wrong-id manifests, and surfaces non-canonical/stale or missing-provenance chains as warnings (never mutates anything). Executable half of [docs/dashboard/dashboard_data_contract.md](docs/dashboard/dashboard_data_contract.md). The dashboard **UI** now lives in `apps/dashboard/` (see below). |
| `apps/dashboard/` | Done (Dashboard v0.1) | Read-only Technical Validation Dashboard UI — first code outside `src/`, a standalone Next.js 16 (App Router, Turbopack) + React 19 + Tailwind v4 + shadcn/ui + Recharts app. App shell (`app-shell`/`sidebar-nav`/`page-header`) + **all five planned views built** on static demo data — Executive Overview (`/overview`), Operational Timeline (`/timeline`), Sensor Health (`/sensor-health`), Drift & Anomaly (`/drift-anomaly`), Incidents (`/incidents`) — each with a dedicated Recharts chart (`src/components/charts/`) and per-view demo payload (`src/lib/dashboard/demo-*.ts`), plus InjexCore theme tokens. Demo-data-driven only — no backend wiring / no Parquet reads yet; **next: real data connection via a read-only FastAPI service** serving dashboard-ready responses (API contract designed in [docs/dashboard/dashboard_api_contract.md](docs/dashboard/dashboard_api_contract.md); no JSON-export layer). Frontend rules: [.claude/rules/frontend/react.md](.claude/rules/frontend/react.md). Stack note: [docs/architecture/InjexCore_Technical_Stack_Dashboard_v0.md](docs/architecture/InjexCore_Technical_Stack_Dashboard_v0.md). |
