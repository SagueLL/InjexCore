...
| `apps/dashboard/` | In progress (Dashboard v0) | Read-only Technical Validation Dashboard UI — first code outside `src/`, a standalone Next.js 16 (App Router, Turbopack) + React 19 + Tailwind v4 + shadcn/ui + Recharts app. App shell (`app-shell`/`sidebar-nav`/`page-header`) + the five planned routes (`/overview`, `/timeline`, `/sensor-health`, `/drift-anomaly`, `/incidents`) scaffolded; **Executive Overview built** on static demo data (`src/lib/dashboard/demo-overview.ts`), the other four are placeholders. Demo-data-driven only — no backend wiring / no Parquet reads yet (planned path: intelligence outputs → dashboard-ready JSON → Next.js). Frontend rules: [.claude/rules/frontend/react.md](.claude/rules/frontend/react.md). Stack note: [docs/architecture/InjexCore_Technical_Stack_Dashboard_v0.md](docs/architecture/InjexCore_Technical_Stack_Dashboard_v0.md). |
...
- **In progress:** Anomaly criterion definition (real-machine thresholds in `.claude/CLAUDE.local.md`); human review of the Iteration C Part 3 decision report (`data/intelligence/forensics/reference_decision/<latest>/decision_summary.md` + `decision_matrix.parquet`) — in particular the pending `inlet_hopper_points` quarantine approval (`approval_required=True, approved=False`) and the deferred (immaterial-residual) Reference v2 candidate. **Dashboard v0 frontend** (`apps/dashboard/` — Next.js 16 / shadcn/ui): app shell + the five planned routes scaffolded and the **Executive Overview** view built on static demo data; lint / `tsc` / `next build` all green. Demo-data-driven only — no backend wiring yet. **Next immediate step: the Operational Timeline view (`/timeline`).** The system is in **technical validation dashboard build, not production deployment**.
- **Not started:** an *approved* Reference v2 (human-gated refit excluding the quarantined channel) and a true PCA/Mahalanobis rescoring with altered feature matrices — both explicitly deferred, gated on the decision report + plant-record review; classification/forecasting models (`src/models/`) — supervised estimators consuming the Intelligence-Layer artifacts; LOF / One-Class SVM benchmarking; temporal forecasting + residual anomaly detection; streaming inference / alert delivery; output API; the remaining **Dashboard v0 views** (`/timeline`, `/sensor-health`, `/drift-anomaly`, `/incidents` — scaffolded as placeholders) and the **dashboard-ready JSON export** that will replace the current static demo data (the contract + lineage validator in `src/dashboard/` are in place; the Next.js UI in `apps/dashboard/` has the shell + Executive Overview only); the broader Python-side visualization (`src/visualization/`).
