...
```
data_generation/ → preprocessing/{cleaning → time_series → feature_engineering → datasets} → intelligence/{behaviour → correlation·pca → anomaly → drift → incidents} → [src/models/] → [src/visualization/]
