...
SRC_DICT = FEATURES_DIR / "data_pellets_dictionary.csv"
DST = FEATURES_DIR / "variable_classification.csv"
...
CLASSIFICATION: dict[str, tuple[str, str, str, str]] = {
    ...
    "feeder_max_speed_sp": (
        ...
        "-",
        "-",
        "Setpoint; input feature for all models",
    ),
    ...
    "conditioner_temp_sp": (
        ...
        "-",
        "-",
        "Setpoint; reference for temp_forecasting residuals",
    ),
    "expander_cone_pressure_sp": (
        "Control",
        "-",
        "-",
        "Setpoint; reference for pressure_forecasting residuals",
    ),
    "granulator_g2_state_pct": (
        "Control",
        "-",
        "-",
        "SCADA load command for granulator",
    ),
    "conditioner_l2_state_pct": (
        "Control",
        "-",
        "-",
        "SCADA load command for conditioner",
    ),
    ...
    "steam_dosing_valve_me2_pct": (
        ...
        "-",
        "-",
        "Valve opening command (manipulated variable)",
    ),
    ...
    "conditioner_inlet_temp": (
        ...
        "anomaly_detection",
        "unsupervised",
        "Inlet thermal condition; baseline for product quality",
    ),
    "granulator_roller_gap": (
        "Process",
        "anomaly_detection",
        "unsupervised",
        "Mechanical state; drift indicator for wear",
    ),
    "granulator_power": (
        "Process",
        "energy_consumption",
        "regression",
        "Main energy consumer; also anomaly feature",
    ),
    "steam_valve_temp_me2": (
        "Process",
        "anomaly_detection",
        "unsupervised",
        "Steam-system thermal signal",
    ),
    "steam_valve_flow_me2": (
        "Process",
        "anomaly_detection",
        "unsupervised",
        "Steam mass flow; key process-stability signal",
    ),
    "steam_valve_pressure_me2": (
        "Process",
        "anomaly_detection",
        "unsupervised",
        "Steam pressure feedback",
    ),
    "granulator_production_rate": (
        "Process",
        "production_rate",
        "regression",
        "Throughput KPI; also energy-efficiency input",
    ),
    "feeder_hopper_temp": (
        "Process",
        "anomaly_detection",
        "unsupervised",
        "Upstream thermal condition",
    ),
    "conditioner_l2_power": (
        "Process",
        "energy_consumption",
        "regression",
        "Energy draw component",
    ),
    "conditioner_steam_loop_temp": (
        "Process",
        "temperature_forecasting",
        "forecasting",
        "Controlled temp; lead indicator for thermal upsets",
    ),
    "expander_ex2_power": (
        "Process",
        "energy_consumption",
    ...
    "expander_ex2_hydraulic_press": (
        "Process",
        "pressure_forecasting",
        "forecasting",
        "Hydraulic state; predictive-maintenance signal",
    ),
    "expander_ex2_outlet_temp": (
        "Process",
        "temperature_forecasting",
        "forecasting",
        "Product outlet temp; quality-relevant",
    ),
    "extruder_specific_energy": (
        "Process",
        "energy_consumption",
        "regression",
        "Primary energy KPI (kWh/t)",
    ),
    "inlet_hopper_humidity": (
        "Process",
        "product_humidity",
        "regression",
        "Quality input; affects pellet integrity",
    ),
    "inlet_hopper_points": (
        "Process",
        "anomaly_detection",
        "unsupervised",
        "Raw count from new hopper sensor; semantics TBD",
    ),
    "inlet_hopper_temp": (
        "Process",
        "temperature_forecasting",
        "forecasting",
        "Upstream product temp; pretreatment indicator",
    ),
    # --- State (discrete equipment states) ---
    "granulator_g2_running": (
        "State",
        "-",
        "-",
        "Operating-mode flag; segment data on/off",
    ),
    "granulator_g2_alarm": (
        "State",
        "machine_failure",
        "classification",
        "Primary failure label for granulator",
    ),
    "conditioner_me2_alarm": (
        "State",
        "machine_failure",
        "classification",
        "Primary failure label for conditioner",
    ),
    ...
    "feeder_al2_alarm": (
        ...
        "machine_failure",
        "classification",
        "Primary failure label for feeder",
    ),
}
...
def main() -> None:
    if not SRC_DICT.exists():
        raise FileNotFoundError(
            f"Dictionary not found: {SRC_DICT}. Run build_pellet_dictionary.py first."
        )
